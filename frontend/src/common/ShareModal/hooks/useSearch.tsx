...
import { Action, ShareOptions, SearchState, ShareAction } from "../type";
import { defaultActions, useSearchInitialState } from "../utils";
...
export const useSearch = ({
  ...
  const [state, dispatch] = useReducer(reducer, useSearchInitialState);
        ...
        .filter(
          (right: { id: string }) =>
            !shareRights.rights.find(
              (shareRight: { id: string }) => shareRight.id === right.id,
            ),
        )
  ...
  const handleSearchResultsChange = async (models: Array<string | number>) => {
    const shareSubject = state.searchAPIResults.find(
      (searchAPIResult) => searchAPIResult.id === models[0],
    );
      ...
      if (shareSubject.type === "sharebookmark") {
        ...
        bookmarkRes.users
          .filter(
            (user: { id: string }) =>
              !shareRights.rights.find(
                (right: { id: string }) => right.id === user.id,
              ),
          )
          .forEach((user: User) => {
            rightsToAddList.push({
        ...
        bookmarkRes.groups
          .filter(
            (group: { id: string }) =>
              !shareRights.rights.find(
                (right: { id: string }) => right.id === group.id,
              ),
          )
          .forEach((group: Group) => {
            rightsToAddList.push({
